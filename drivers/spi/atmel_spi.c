...
static int atmel_spi_disable_clk(struct udevice *bus)
{
	struct clk clk;
	int ret;
...
static int atmel_spi_remove(struct udevice *bus)
{
	struct atmel_spi_plat *bus_plat = dev_get_plat(bus);

	writel(ATMEL_SPI_CR_SWRST, &bus_plat->regs->cr);

	atmel_spi_disable_clk(bus);

#if CONFIG_IS_ENABLED(DM_GPIO)
	struct atmel_spi_priv *priv = dev_get_priv(bus);

	gpio_free_list(bus, priv->cs_gpios, ARRAY_SIZE(priv->cs_gpios));
#endif
